...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
        Object objM3349constructorimpl;
        boolean z;
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        objM3349constructorimpl = SegmentOrClosed.m3349constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    objM3349constructorimpl = SegmentOrClosed.m3349constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (SegmentOrClosed.m3354isClosedimpl(objM3349constructorimpl)) {
                break;
            }
            Segment segmentM3352getSegmentimpl = SegmentOrClosed.m3352getSegmentimpl(objM3349constructorimpl);
            while (true) {
                Segment segment = (Segment) this.tail;
                if (segment.getId() >= segmentM3352getSegmentimpl.getId()) {
                    break;
                }
                if (!segmentM3352getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                    z = false;
                    break;
                }
                if (C0136xc40028dd.m105m(tail$FU, this, segment, segmentM3352getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (segmentM3352getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                    segmentM3352getSegmentimpl.remove();
                }
            }
            z = true;
        } while (!z);
        SemaphoreSegment semaphoreSegment3 = (SemaphoreSegment) SegmentOrClosed.m3352getSegmentimpl(objM3349constructorimpl);
        int i = (int) (andIncrement % ((long) SemaphoreKt.SEGMENT_SIZE));
        if (ConcurrentWeakMap$Core$$ExternalSyntheticBackportWithForwarding0.m1321m(semaphoreSegment3.acquirers, i, null, cont)) {
            cont.invokeOnCancellation(new CancelSemaphoreAcquisitionHandler(semaphoreSegment3, i));
            return true;
        }
        if (ConcurrentWeakMap$Core$$ExternalSyntheticBackportWithForwarding0.m1321m(semaphoreSegment3.acquirers, i, SemaphoreKt.PERMIT, SemaphoreKt.TAKEN)) {
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        do {
            ...
            while (true) {
                ...
                if (C0136xc40028dd.m105m(head$FU, this, segment, segmentM3352getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (segmentM3352getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
        ...
        if (andSet == null) {
            ...
            return !ConcurrentWeakMap$Core$$ExternalSyntheticBackportWithForwarding0.m1321m(semaphoreSegment3.acquirers, i2, SemaphoreKt.PERMIT, SemaphoreKt.BROKEN);
        }
