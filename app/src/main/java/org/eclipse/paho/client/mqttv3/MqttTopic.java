    ...
    public static void validate(String str, boolean z) throws IllegalArgumentException {
        try {
            ...
            if (z) {
                ...
                if (Strings.countMatches(str, MULTI_LEVEL_WILDCARD) > 1 || (str.indexOf(MULTI_LEVEL_WILDCARD) != -1 && !str.endsWith(MULTI_LEVEL_WILDCARD_PATTERN))) {
    ...
    private static void validateSingleLevelWildcard(String str) {
        ...
        while (i < length) {
            ...
            if (charArray[i] == cCharAt && ((c != cCharAt2 && c != 0) || (c2 != cCharAt2 && c2 != 0))) {
                throw new IllegalArgumentException(new StringBuffer("Invalid usage of single-level wildcard in topic string '").append(str).append("'!").toString());
            }
            i = i3;
        }
    }
