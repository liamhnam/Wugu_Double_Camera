...
public class ClientState {
    ...
    private String getReceivedPersistenceKey(int i) {
        return new StringBuffer(PERSISTENCE_RECEIVED_PREFIX).append(i).toString();
    }

    private String getSendBufferedPersistenceKey(MqttWireMessage mqttWireMessage) {
        return new StringBuffer(PERSISTENCE_SENT_BUFFERED_PREFIX).append(mqttWireMessage.getMessageId()).toString();
    }
                ...
                synchronized (obj2) {
                    try {
                        try {
                            ...
                            if (i > 0) {
                                obj = obj2;
                                if (jCurrentTimeMillis - this.lastInboundActivity >= this.keepAlive + ((long) 100)) {
                                    logger.severe(str, "checkForActivity", "619", new Object[]{new Long(this.keepAlive), new Long(this.lastOutboundActivity), new Long(this.lastInboundActivity), new Long(jCurrentTimeMillis), new Long(this.lastPing)});
                                    throw ExceptionHelper.createMqttException(32000);
                                }
                            } else {
                                obj = obj2;
                            }
                            if (i == 0 && jCurrentTimeMillis - this.lastOutboundActivity >= this.keepAlive * 2) {
                                logger.severe(str, "checkForActivity", "642", new Object[]{new Long(this.keepAlive), new Long(this.lastOutboundActivity), new Long(this.lastInboundActivity), new Long(jCurrentTimeMillis), new Long(this.lastPing)});
                                throw ExceptionHelper.createMqttException(32002);
                            }
                            if ((i == 0 && jCurrentTimeMillis - this.lastInboundActivity >= this.keepAlive - ((long) 100)) || jCurrentTimeMillis - this.lastOutboundActivity >= this.keepAlive - ((long) 100)) {
