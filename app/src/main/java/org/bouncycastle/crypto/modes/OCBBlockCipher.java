...
public class OCBBlockCipher implements AEADBlockCipher {
    ...
    protected static byte[] OCB_double(byte[] bArr) {
        ...
        bArr2[15] = (byte) ((TsExtractor.TS_STREAM_TYPE_E_AC3 >>> ((1 - iShiftLeft) << 3)) ^ bArr2[15]);
        return bArr2;
    }
    ...
    protected byte[] getLSub(int i) {
        while (i >= this.f3308L.size()) {
            Vector vector = this.f3308L;
            vector.addElement(OCB_double((byte[]) vector.lastElement()));
        }
        return (byte[]) this.f3308L.elementAt(i);
    }
    ...
    protected int processNonce(byte[] bArr) {
        ...
        bArr2[15] = (byte) (b & SnmpConstants.ASN_PRIVATE);
