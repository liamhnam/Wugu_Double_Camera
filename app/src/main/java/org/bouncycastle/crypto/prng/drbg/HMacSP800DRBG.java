...
public class HMacSP800DRBG implements SP80090DRBG {
    ...
    private byte[] f3408_K;

    private byte[] f3409_V;
    ...
    private void hmac_DRBG_Update_Func(byte[] bArr, byte b) {
        this._hMac.init(new KeyParameter(this.f3408_K));
        Mac mac = this._hMac;
        byte[] bArr2 = this.f3409_V;
        mac.update(bArr2, 0, bArr2.length);
        this._hMac.update(b);
        if (bArr != null) {
            this._hMac.update(bArr, 0, bArr.length);
        }
        this._hMac.doFinal(this.f3408_K, 0);
        this._hMac.init(new KeyParameter(this.f3408_K));
        Mac mac2 = this._hMac;
        byte[] bArr3 = this.f3409_V;
        mac2.update(bArr3, 0, bArr3.length);
        this._hMac.doFinal(this.f3409_V, 0);
    }
