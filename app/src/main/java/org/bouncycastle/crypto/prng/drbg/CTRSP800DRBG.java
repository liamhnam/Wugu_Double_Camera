...
public class CTRSP800DRBG implements SP80090DRBG {
    ...
    private void CTR_DRBG_Reseed_algorithm(byte[] bArr) {
        CTR_DRBG_Update(Block_Cipher_df(Arrays.concatenate(getEntropy(), bArr), this._seedLength), this._Key, this.f3402_V);
        this._reseedCounter = 1L;
    }
    ...
    private int getMaxSecurityStrength(BlockCipher blockCipher, int i) {
        if (isTDEA(blockCipher) && i == 168) {
            return UiPosIndexEnum.HOME_COUNTDOWN;
        }
    ...
    private void padKey(byte[] bArr, int i, byte[] bArr2, int i2) {
        ...
        int i11 = bArr[i10] << 3;
        int i12 = i + 5;
        bArr2[i2 + 5] = (byte) (i11 | ((bArr[i12] & SnmpConstants.ASN_PRIVATE) >>> 5));
        int i13 = i + 6;
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) {
        ...
        if (bArr2 != null) {
            ...
            CTR_DRBG_Update(bArrBlock_Cipher_df, this._Key, this.f3402_V);
        } else {
            bArrBlock_Cipher_df = new byte[this._seedLength / 8];
        }
        int length = this.f3402_V.length;
        byte[] bArr3 = new byte[length];
        this._engine.init(true, new KeyParameter(expandKey(this._Key)));
        for (int i = 0; i <= bArr.length / length; i++) {
            int i2 = i * length;
            int length2 = bArr.length - i2 > length ? length : bArr.length - (this.f3402_V.length * i);
            if (length2 != 0) {
                addOneTo(this.f3402_V);
                this._engine.processBlock(this.f3402_V, 0, bArr3, 0);
