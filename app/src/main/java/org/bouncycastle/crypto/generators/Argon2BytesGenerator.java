...
public class Argon2BytesGenerator {
    ...
    private static class Block {
        ...
        private final long[] f3223v;

        private Block() {
            this.f3223v = new long[128];
        }

        public void copyBlock(Block block) {
            System.arraycopy(block.f3223v, 0, this.f3223v, 0, 128);
        }

        public void xor(Block block, Block block2) {
            long[] jArr = this.f3223v;
            long[] jArr2 = block.f3223v;
            long[] jArr3 = block2.f3223v;
            for (int i = 0; i < 128; i++) {
        ...
        void fromBytes(byte[] bArr) {
            ...
            Pack.littleEndianToLong(bArr, 0, this.f3223v);
        }
        ...
        void toBytes(byte[] bArr) {
            ...
            Pack.longToLittleEndian(this.f3223v, bArr, 0);
        }
    }
    ...
    private static class FillBlock {
        ...
        private FillBlock() {
            this.f3224R = new Block();
            this.f3225Z = new Block();
        ...
        private void applyBlake() {
            for (int i = 0; i < 8; i++) {
                int i2 = i * 16;
                Argon2BytesGenerator.roundFunction(this.f3225Z, i2, i2 + 1, i2 + 2, i2 + 3, i2 + 4, i2 + 5, i2 + 6, i2 + 7, i2 + 8, i2 + 9, i2 + 10, i2 + 11, i2 + 12, i2 + 13, i2 + 14, i2 + 15);
            }
            for (int i3 = 0; i3 < 8; i3++) {
                int i4 = i3 * 2;
                Argon2BytesGenerator.roundFunction(this.f3225Z, i4, i4 + 1, i4 + 16, i4 + 17, i4 + 32, i4 + 33, i4 + 48, i4 + 49, i4 + 64, i4 + 65, i4 + 80, i4 + 81, i4 + 96, i4 + 97, i4 + UiPosIndexEnum.HOME_COUNTDOWN, i4 + UiPosIndexEnum.HOME_REPLACE_BG_TAB);
            }
        }

        public void fillBlock(Block block, Block block2) {
            this.f3225Z.copyBlock(block);
            applyBlake();
            block2.xor(block, this.f3225Z);
        }

        public void fillBlock(Block block, Block block2, Block block3) {
            this.f3224R.xor(block, block2);
            this.f3225Z.copyBlock(this.f3224R);
            applyBlake();
            block3.xor(this.f3224R, this.f3225Z);
        }
        ...
        public void fillBlockWithXor(Block block, Block block2, Block block3) {
            ...
            this.f3225Z.copyBlock(this.f3224R);
            applyBlake();
            block3.xorWith(this.f3224R, this.f3225Z);
        }
    }
    ...
    private static void m1438F(long[] jArr, int i, int i2, int i3, int i4) {
    ...
    private long getPseudoRandom(FillBlock fillBlock, int i, Block block, Block block2, int i2, boolean z) {
        if (!z) {
            return this.memory[i2].f3223v[0];
        }
        int i3 = i % 128;
        if (i3 == 0) {
            nextAddresses(fillBlock, block2, block);
        }
        return block.f3223v[i3];
    }
    ...
    private void initAddressBlocks(FillBlock fillBlock, Position position, Block block, Block block2) {
        block.f3223v[0] = intToLong(position.pass);
        block.f3223v[1] = intToLong(position.lane);
        block.f3223v[2] = intToLong(position.slice);
        block.f3223v[3] = intToLong(this.memory.length);
        block.f3223v[4] = intToLong(this.parameters.getIterations());
        block.f3223v[5] = intToLong(this.parameters.getType());
    ...
    public static void roundFunction(Block block, int i, int i2, int i3, int i4, int i5, int i6, int i7, int i8, int i9, int i10, int i11, int i12, int i13, int i14, int i15, int i16) {
        long[] jArr = block.f3223v;
        m1438F(jArr, i, i5, i9, i13);
        m1438F(jArr, i2, i6, i10, i14);
        m1438F(jArr, i3, i7, i11, i15);
        m1438F(jArr, i4, i8, i12, i16);
        m1438F(jArr, i, i6, i11, i16);
        m1438F(jArr, i2, i7, i12, i13);
        m1438F(jArr, i3, i8, i9, i14);
        m1438F(jArr, i4, i5, i10, i15);
    }
