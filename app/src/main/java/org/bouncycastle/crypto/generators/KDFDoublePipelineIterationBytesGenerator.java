...
public class KDFDoublePipelineIterationBytesGenerator implements MacDerivationFunction {
    ...
    public KDFDoublePipelineIterationBytesGenerator(Mac mac) {
        ...
        this.f3235h = macSize;
        this.f3234a = new byte[macSize];
        this.f3236k = new byte[macSize];
    }
    ...
    private void generateNext() {
        if (this.generatedBytes == 0) {
            ...
            this.prf.doFinal(this.f3234a, 0);
        } else {
            Mac mac2 = this.prf;
            byte[] bArr2 = this.f3234a;
            mac2.update(bArr2, 0, bArr2.length);
            this.prf.doFinal(this.f3234a, 0);
        }
        Mac mac3 = this.prf;
        byte[] bArr3 = this.f3234a;
        mac3.update(bArr3, 0, bArr3.length);
        if (this.useCounter) {
            int i = (this.generatedBytes / this.f3235h) + 1;
            byte[] bArr4 = this.ios;
        ...
        this.prf.doFinal(this.f3236k, 0);
    }
    ...
    public int generateBytes(byte[] bArr, int i, int i2) throws DataLengthException, IllegalArgumentException {
        ...
        System.arraycopy(this.f3236k, i7, bArr, i, iMin);
        ...
        while (true) {
            ...
            generateNext();
            iMin = Math.min(this.f3235h, i8);
            System.arraycopy(this.f3236k, 0, bArr, i, iMin);
            this.generatedBytes += iMin;
            i8 -= iMin;
        }
    }
    ...
    public void init(DerivationParameters derivationParameters) {
        ...
        if (kDFDoublePipelineIterationParameters.useCounter()) {
            BigInteger bigIntegerMultiply = TWO.pow(r).multiply(BigInteger.valueOf(this.f3235h));
