...
public class DSAParametersGenerator {
    ...
    private static BigInteger calculateGenerator_FIPS186_3_Verifiable(Digest digest, BigInteger bigInteger, BigInteger bigInteger2, byte[] bArr, int i) {
        ...
        int length = bArr.length + bArrDecodeStrict.length + 1 + 2;
        ...
        while (true) {
            ...
            if (isProbablePrime(bigInteger)) {
                ...
                while (i6 < 4096) {
                    ...
                    if (bigIntegerSubtract.bitLength() == this.f3230L && isProbablePrime(bigIntegerSubtract)) {
    ...
    private DSAParameters generateParameters_FIPS186_3() {
        ...
        byte[] bArr = new byte[this.f3231N / 8];
        int i2 = this.f3230L;
        ...
        loop0: while (true) {
            ...
            bit = new BigInteger(1, bArr3).mod(ONE.shiftLeft(this.f3231N - 1)).setBit(0).setBit(this.f3231N - 1);
            if (isProbablePrime(bit)) {
                byte[] bArrClone = Arrays.clone(bArr);
                int i7 = this.f3230L * 4;
                i = 0;
                while (i < i7) {
    ...
    public void init(int i, int i2, SecureRandom secureRandom) {
        this.f3230L = i;
        this.f3231N = getDefaultN(i);
        this.certainty = i2;
        this.iterations = Math.max(getMinimumIterations(this.f3230L), (i2 + 1) / 2);
