...
public class ISO9796d1Encoding implements AsymmetricBlockCipher {
    ...
    private static byte[] shadows = {14, 3, 5, 8, 9, 4, 2, SnmpConstants.SNMP_ERR_UNDOFAILED, 0, 13, 11, 6, 7, 10, 12, 1};
    private static byte[] inverse = {8, SnmpConstants.SNMP_ERR_UNDOFAILED, 6, 1, 5, 2, 11, 12, 3, 4, 13, 10, 14, 9, 0, 7};
    ...
    private byte[] decodeBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        ...
        if ((bArrConvertOutputDecryptOnly[bArrConvertOutputDecryptOnly.length - 1] & SnmpConstants.SNMP_ERR_UNDOFAILED) != 6) {
        ...
        byte b2 = (byte) (bArr2[b & SnmpConstants.SNMP_ERR_UNDOFAILED] | (bArr2[(b & UByte.MAX_VALUE) >>> 4] << 4));
        ...
        for (int length = bArrConvertOutputDecryptOnly.length - 1; length >= bArrConvertOutputDecryptOnly.length - (i3 * 2); length -= 2) {
            ...
            int i6 = bArr3[b3 & SnmpConstants.SNMP_ERR_UNDOFAILED] | (bArr3[(b3 & UByte.MAX_VALUE) >>> 4] << 4);
            int i7 = length - 1;
    ...
    private byte[] encodeBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        ...
        for (int i10 = i4 - (i7 * 2); i10 != i4; i10 += 2) {
            ...
            byte[] bArr3 = shadows;
            bArr2[i10] = (byte) (bArr3[b & SnmpConstants.SNMP_ERR_UNDOFAILED] | (bArr3[(b & UByte.MAX_VALUE) >>> 4] << 4));
            bArr2[i10 + 1] = b;
        }
        int i11 = i4 - (i2 * 2);
