...
public class RC2WrapEngine implements Wrapper {
    private static final byte[] IV2 = {74, -35, SnmpConstants.GET_RSP_MSG, 44, 121, -24, 33, 5};
    private CBCBlockCipher engine;
    private boolean forWrapping;

    private byte[] f3180iv;
    private CipherParameters param;
    private ParametersWithIV paramPlusIV;

    private SecureRandom f3181sr;
    ...
    public void init(boolean z, CipherParameters cipherParameters) {
        ...
        if (cipherParameters instanceof ParametersWithRandom) {
            ...
            this.f3181sr = parametersWithRandom.getRandom();
            cipherParameters = parametersWithRandom.getParameters();
        } else {
            this.f3181sr = CryptoServicesRegistrar.getSecureRandom();
        }
        if (!(cipherParameters instanceof ParametersWithIV)) {
            ...
            if (this.forWrapping) {
                ...
                this.f3180iv = bArr;
                this.f3181sr.nextBytes(bArr);
                this.paramPlusIV = new ParametersWithIV(this.param, this.f3180iv);
                return;
            }
            return;
        }
        ...
        this.f3180iv = parametersWithIV.getIV();
        this.param = this.paramPlusIV.getParameters();
        if (!this.forWrapping) {
            throw new IllegalArgumentException("You should not supply an IV for unwrapping");
        }
        byte[] bArr2 = this.f3180iv;
    ...
    public byte[] unwrap(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        ...
        ParametersWithIV parametersWithIV = new ParametersWithIV(this.param, this.f3180iv);
        this.paramPlusIV = parametersWithIV;
    ...
    public byte[] wrap(byte[] bArr, int i, int i2) {
        ...
        byte[] bArr6 = this.f3180iv;
        int length2 = bArr6.length + length;
        byte[] bArr7 = new byte[length2];
        System.arraycopy(bArr6, 0, bArr7, 0, bArr6.length);
        System.arraycopy(bArr5, 0, bArr7, this.f3180iv.length, length);
