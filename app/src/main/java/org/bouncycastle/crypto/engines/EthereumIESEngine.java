...
public class EthereumIESEngine {
    ...
    public static class HandshakeKDFFunction implements DigestDerivationFunction {
        ...
        public void init(DerivationParameters derivationParameters) {
            if (derivationParameters instanceof KDFParameters) {
                ...
                this.f3165iv = kDFParameters.getIV();
            } else {
    ...
    private byte[] decryptBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        ...
        byte[] bArr3;
        int iProcessBytes;
        if (i2 < this.f3164V.length + this.mac.getMacSize()) {
            throw new InvalidCipherTextException("length of input must be greater than the MAC and V combined");
        }
        if (this.cipher == null) {
            int length = (i2 - this.f3164V.length) - this.mac.getMacSize();
            ...
            for (int i4 = 0; i4 != length; i4++) {
                bArr3[i4] = (byte) (bArr[(this.f3164V.length + i) + i4] ^ bArr4[i4]);
            }
            iProcessBytes = 0;
        } else {
            ...
            if (this.f3163IV != null) {
                keyParameter = new ParametersWithIV(keyParameter, this.f3163IV);
            }
            this.cipher.init(false, keyParameter);
            bArr3 = new byte[this.cipher.getOutputSize((i2 - this.f3164V.length) - this.mac.getMacSize())];
            BufferedBlockCipher bufferedBlockCipher = this.cipher;
            byte[] bArr8 = this.f3164V;
            iProcessBytes = bufferedBlockCipher.processBytes(bArr, i + bArr8.length, (i2 - bArr8.length) - this.mac.getMacSize(), bArr3, 0);
        }
        byte[] encodingV = this.param.getEncodingV();
        byte[] lengthTag = this.f3164V.length != 0 ? getLengthTag(encodingV) : null;
        int i6 = i + i2;
        ...
        Mac mac = this.mac;
        byte[] bArr11 = this.f3163IV;
        mac.update(bArr11, 0, bArr11.length);
        Mac mac2 = this.mac;
        byte[] bArr12 = this.f3164V;
    ...
    private byte[] encryptBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        ...
        if (this.cipher == null) {
            ...
            if (this.f3163IV != null) {
                ...
                keyParameter = new ParametersWithIV(new KeyParameter(bArr6), this.f3163IV);
            } else {
                bufferedBlockCipher = this.cipher;
        ...
        this.mac.doFinal(bArr9, 0);
        byte[] bArr13 = this.f3164V;
        byte[] bArr14 = new byte[bArr13.length + i2 + macSize];
        System.arraycopy(bArr13, 0, bArr14, 0, bArr13.length);
        System.arraycopy(bArr2, 0, bArr14, this.f3164V.length, i2);
        System.arraycopy(bArr9, 0, bArr14, this.f3164V.length + i2, macSize);
        return bArr14;
    }
    ...
    private void extractParams(CipherParameters cipherParameters) {
        if (cipherParameters instanceof ParametersWithIV) {
            ...
            this.f3163IV = parametersWithIV.getIV();
    ...
    public byte[] processBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        if (this.forEncryption) {
            ...
            if (ephemeralKeyPairGenerator != null) {
                ...
                this.f3164V = ephemeralKeyPairGenerate.getEncodedPublicKey();
            }
        } else if (this.keyParser != null) {
            ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream(bArr, i, i2);
            try {
                this.pubParam = this.keyParser.readKey(byteArrayInputStream);
                this.f3164V = Arrays.copyOfRange(bArr, i, (i2 - byteArrayInputStream.available()) + i);
            } catch (IOException e) {
