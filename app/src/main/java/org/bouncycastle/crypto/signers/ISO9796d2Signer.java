...
public class ISO9796d2Signer implements SignerWithRecovery {
    ...
    public void updateWithRecoveredMessage(byte[] bArr) throws InvalidCipherTextException {
        ...
        if (((bArrProcessBlock[0] & SnmpConstants.ASN_PRIVATE) ^ 64) != 0) {
            throw new InvalidCipherTextException("malformed signature");
        }
        if (((bArrProcessBlock[bArrProcessBlock.length - 1] & SnmpConstants.SNMP_ERR_UNDOFAILED) ^ 12) != 0) {
            throw new InvalidCipherTextException("malformed signature");
        }
        ...
        int i3 = 0;
        while (i3 != bArrProcessBlock.length && ((bArrProcessBlock[i3] & SnmpConstants.SNMP_ERR_UNDOFAILED) ^ 10) != 0) {
            i3++;
        }
        int i4 = i3 + 1;
    ...
    public boolean verifySignature(byte[] bArr) {
        ...
        if (((bArrProcessBlock[0] & SnmpConstants.ASN_PRIVATE) ^ 64) == 0 && ((bArrProcessBlock[bArrProcessBlock.length - 1] & SnmpConstants.SNMP_ERR_UNDOFAILED) ^ 12) == 0) {
            if (((bArrProcessBlock[bArrProcessBlock.length - 1] & UByte.MAX_VALUE) ^ 188) == 0) {
                i = 1;
            } else {
                i = 2;
