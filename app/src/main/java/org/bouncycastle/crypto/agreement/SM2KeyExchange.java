...
public class SM2KeyExchange {
    ...
    private byte[] m1343S1(Digest digest, ECPoint eCPoint, byte[] bArr) {
    ...
    private byte[] m1344S2(Digest digest, ECPoint eCPoint, byte[] bArr) {
    ...
    private BigInteger reduce(BigInteger bigInteger) {
        return bigInteger.and(BigInteger.valueOf(1L).shiftLeft(this.f2921w).subtract(BigInteger.valueOf(1L))).setBit(this.f2921w);
    }
    ...
    public byte[][] calculateKeyWithConfirmation(int i, byte[] bArr, CipherParameters cipherParameters) {
        ...
        if (!this.initiator) {
            ...
            return new byte[][]{bArrKdf, m1343S1(this.digest, eCPointCalculateU, bArrCalculateInnerHash), m1344S2(this.digest, eCPointCalculateU, bArrCalculateInnerHash)};
        }
        ...
        if (Arrays.constantTimeAreEqual(m1343S1(this.digest, eCPointCalculateU, bArrCalculateInnerHash2), bArr)) {
            return new byte[][]{bArrKdf2, m1344S2(this.digest, eCPointCalculateU, bArrCalculateInnerHash2)};
        }
    ...
    public void init(CipherParameters cipherParameters) {
        ...
        this.f2921w = (this.ecParams.getCurve().getFieldSize() / 2) - 1;
    }
}
