...
public class ASN1StreamParser {
    ...
    public ASN1Encodable readObject() throws IOException {
        ...
        if (length < 0) {
            ...
            return (i & 192) == 192 ? new BERPrivateParser(tagNumber, aSN1StreamParser) : (i & 64) != 0 ? new BERApplicationSpecificParser(tagNumber, aSN1StreamParser) : (i & 128) != 0 ? new BERTaggedObjectParser(true, tagNumber, aSN1StreamParser) : aSN1StreamParser.readIndef(tagNumber);
        }
        DefiniteLengthInputStream definiteLengthInputStream = new DefiniteLengthInputStream(this._in, length, this._limit);
        if ((i & 192) == 192) {
            return new DLPrivate(z, tagNumber, definiteLengthInputStream.toByteArray());
        }
        if ((i & 64) != 0) {
