...
public class ASN1GeneralizedTime extends ASN1Primitive {
    ...
    private String convert(int i) {
        return i < 10 ? PDLayoutAttributeObject.GLYPH_ORIENTATION_VERTICAL_ZERO_DEGREES + i : Integer.toString(i);
    }
    ...
    private String pruneFractionalSeconds(String str) {
        ...
        if (i2 > 3) {
            ...
            str2 = strSubstring.substring(0, i) + PDLayoutAttributeObject.GLYPH_ORIENTATION_VERTICAL_ZERO_DEGREES + strSubstring.substring(i);
            sb = new StringBuilder();
        }
    ...
    public String getTime() {
        ...
        if (strFromByteArray.charAt(strFromByteArray.length() - 1) != 'Z') {
            ...
            if (cCharAt2 == '-' || cCharAt2 == '+') {
                int i = length2 + 3;
                sbAppend = new StringBuilder().append(strFromByteArray.substring(0, length2)).append("GMT").append(strFromByteArray.substring(length2, i)).append(":");
                strSubstring = strFromByteArray.substring(i);
            } else {
                int length3 = strFromByteArray.length() - 3;
                char cCharAt3 = strFromByteArray.charAt(length3);
                if (cCharAt3 == '-' || cCharAt3 == '+') {
                    sbAppend3 = new StringBuilder().append(strFromByteArray.substring(0, length3)).append("GMT").append(strFromByteArray.substring(length3));
                    str = ":00";
                } else {
                    sbAppend = new StringBuilder().append(strFromByteArray);
                    strSubstring = calculateGMTOffset(strFromByteArray);
                }
            }
            sbAppend2 = sbAppend.append(strSubstring);
            return sbAppend2.toString();
        }
        sbAppend3 = new StringBuilder().append(strFromByteArray.substring(0, strFromByteArray.length() - 1));
        str = "GMT+00:00";
