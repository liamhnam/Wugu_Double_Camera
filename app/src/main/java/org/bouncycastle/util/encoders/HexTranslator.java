...
public class HexTranslator implements Translator {
    private static final byte[] hexTable = {SnmpConstants.CONS_SEQ, 49, 50, 51, 52, 53, 54, 55, 56, 57, 97, 98, 99, 100, 101, 102};
    ...
    public int encode(byte[] bArr, int i, int i2, byte[] bArr2, int i3) {
        ...
        while (i4 < i2) {
            ...
            bArr2[i6 + 1] = bArr3[bArr[i] & SnmpConstants.SNMP_ERR_UNDOFAILED];
            i++;
            i4++;
            i5 += 2;
        }
