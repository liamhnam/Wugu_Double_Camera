...
public class JcaPublicKeyConverter {
    ...
    private static ECCurve convertCurve(EllipticCurve ellipticCurve, BigInteger bigInteger, int i) {
        ...
        if (field instanceof ECFieldFp) {
            return new ECCurve.C3160Fp(((ECFieldFp) field).getP(), a2, b, bigInteger, BigInteger.valueOf(i));
        }
    ...
    private ECParameterSpec getParams(ECDSAPublicKey eCDSAPublicKey) {
        ...
        ECCurve.C3160Fp c3160Fp = new ECCurve.C3160Fp(eCDSAPublicKey.getPrimeModulusP(), eCDSAPublicKey.getFirstCoefA(), eCDSAPublicKey.getSecondCoefB(), eCDSAPublicKey.getOrderOfBasePointR(), eCDSAPublicKey.getCofactorF());
        ECPoint eCPointDecodePoint = c3160Fp.decodePoint(eCDSAPublicKey.getBasePointG());
        return new ECParameterSpec(convertCurve(c3160Fp), new java.security.spec.ECPoint(eCPointDecodePoint.getAffineXCoord().toBigInteger(), eCPointDecodePoint.getAffineYCoord().toBigInteger()), eCDSAPublicKey.getOrderOfBasePointR(), eCDSAPublicKey.getCofactorF().intValue());
    }
    ...
    private java.security.spec.ECPoint getPublicPoint(ECDSAPublicKey eCDSAPublicKey) {
        ...
        ECPoint.C3163Fp c3163Fp = (ECPoint.C3163Fp) new ECCurve.C3160Fp(eCDSAPublicKey.getPrimeModulusP(), eCDSAPublicKey.getFirstCoefA(), eCDSAPublicKey.getSecondCoefB(), eCDSAPublicKey.getOrderOfBasePointR(), eCDSAPublicKey.getCofactorF()).decodePoint(eCDSAPublicKey.getPublicPointY());
        return new java.security.spec.ECPoint(c3163Fp.getAffineXCoord().toBigInteger(), c3163Fp.getAffineYCoord().toBigInteger());
    }

    public PublicKey getKey(PublicKeyDataObject publicKeyDataObject) throws EACException, InvalidKeySpecException {
        if (publicKeyDataObject.getUsage().m1338on(EACObjectIdentifiers.id_TA_ECDSA)) {
