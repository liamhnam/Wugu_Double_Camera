...
public class X509CertificateObject extends X509Certificate implements PKCS12BagAttributeCarrier {
    ...
    private void checkSignature(PublicKey publicKey, Signature signature) throws NoSuchAlgorithmException, SignatureException, InvalidKeyException, CertificateException {
        if (!isAlgIdEqual(this.f3472c.getSignatureAlgorithm(), this.f3472c.getTBSCertificate().getSignature())) {
            throw new CertificateException("signature algorithm in TBS cert not same as outer cert");
        }
        X509SignatureUtil.setSignatureParameters(signature, this.f3472c.getSignatureAlgorithm().getParameters());
    ...
    private byte[] getExtensionBytes(String str) {
        Extension extension;
        Extensions extensions = this.f3472c.getTBSCertificate().getExtensions();
    ...
    public void checkValidity(Date date) throws CertificateNotYetValidException, CertificateExpiredException {
        if (date.getTime() > getNotAfter().getTime()) {
            throw new CertificateExpiredException("certificate expired on " + this.f3472c.getEndDate().getTime());
        }
        if (date.getTime() < getNotBefore().getTime()) {
            throw new CertificateNotYetValidException("certificate not valid till " + this.f3472c.getStartDate().getTime());
        }
    }
    ...
    public byte[] getEncoded() throws CertificateEncodingException {
        try {
            return this.f3472c.getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
    ...
    public Principal getIssuerDN() {
        return new X509Principal(this.f3472c.getIssuer());
    }

    @Override
    public boolean[] getIssuerUniqueID() {
        DERBitString issuerUniqueId = this.f3472c.getTBSCertificate().getIssuerUniqueId();
    ...
    public X500Principal getIssuerX500Principal() {
        try {
            return new X500Principal(this.f3472c.getIssuer().getEncoded());
    ...
    public Date getNotAfter() {
        return this.f3472c.getEndDate().getDate();
    }

    @Override
    public Date getNotBefore() {
        return this.f3472c.getStartDate().getDate();
    }

    @Override
    public PublicKey getPublicKey() {
        try {
            return BouncyCastleProvider.getPublicKey(this.f3472c.getSubjectPublicKeyInfo());
        } catch (IOException unused) {
            return null;
        }
    }
    ...
    public BigInteger getSerialNumber() {
        return this.f3472c.getSerialNumber().getValue();
    }
    ...
    public String getSigAlgOID() {
        return this.f3472c.getSignatureAlgorithm().getAlgorithm().getId();
    }

    @Override
    public byte[] getSigAlgParams() {
        if (this.f3472c.getSignatureAlgorithm().getParameters() != null) {
            try {
                return this.f3472c.getSignatureAlgorithm().getParameters().toASN1Primitive().getEncoded(ASN1Encoding.DER);
            } catch (IOException unused) {
            }
        }
        return null;
    ...
    public byte[] getSignature() {
        return this.f3472c.getSignature().getOctets();
    }
    ...
    public Principal getSubjectDN() {
        return new X509Principal(this.f3472c.getSubject());
    }

    @Override
    public boolean[] getSubjectUniqueID() {
        DERBitString subjectUniqueId = this.f3472c.getTBSCertificate().getSubjectUniqueId();
    ...
    public X500Principal getSubjectX500Principal() {
        try {
            return new X500Principal(this.f3472c.getSubject().getEncoded());
        } catch (IOException unused) {
            throw new IllegalStateException("can't encode issuer DN");
        }
    }
    ...
    public byte[] getTBSCertificate() throws CertificateEncodingException {
        try {
            return this.f3472c.getTBSCertificate().getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
            throw new CertificateEncodingException(e.toString());
        }
    }
    ...
    public int getVersion() {
        return this.f3472c.getVersionNumber();
    }

    @Override
    public boolean hasUnsupportedCriticalExtension() {
        Extensions extensions;
        if (getVersion() != 3 || (extensions = this.f3472c.getTBSCertificate().getExtensions()) == null) {
            return false;
        }
        Enumeration enumerationOids = extensions.oids();
        while (enumerationOids.hasMoreElements()) {
    ...
    public final void verify(PublicKey publicKey) throws NoSuchAlgorithmException, SignatureException, InvalidKeyException, CertificateException, NoSuchProviderException {
        Signature signature;
        String signatureName = X509SignatureUtil.getSignatureName(this.f3472c.getSignatureAlgorithm());
        try {
