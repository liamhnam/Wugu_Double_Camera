...
public class SPHINCS256Signer implements MessageSigner {
    ...
    byte[] crypto_sign(HashFunctions hashFunctions, byte[] bArr, byte[] bArr2) {
        ...
        System.arraycopy(bArr9, Status.Code.clientErrorNotFetchable, bArr3, 40968, 32);
    ...
    boolean verify(HashFunctions hashFunctions, byte[] bArr, byte[] bArr2, byte[] bArr3) {
        ...
        byte[] bArr8 = new byte[Status.Code.clientErrorNotFetchable];
        if (length != 41000) {
        ...
        messageHash.update(bArr8, 0, Status.Code.clientErrorNotFetchable);
        messageHash.update(bArr, 0, bArr.length);
        messageHash.doFinal(bArr9, 0);
        long j = 0;
        for (int i3 = 0; i3 < 8; i3++) {
            j ^= ((long) (bArr7[32 + i3] & UByte.MAX_VALUE)) << (i3 * 8);
        }
        new Horst();
