...
class TouchResponse {
    ...
    private void fillFromAttributeList(Context context, AttributeSet attrs) {
        TypedArray typedArrayObtainStyledAttributes = context.obtainStyledAttributes(attrs, C0179R.styleable.OnSwipe);
    ...
    private void fill(TypedArray a2) {
        ...
        for (int i = 0; i < indexCount; i++) {
            ...
            } else if (index == C0179R.styleable.OnSwipe_maxVelocity) {
                this.mMaxVelocity = a2.getFloat(index, this.mMaxVelocity);
            } else if (index == C0179R.styleable.OnSwipe_maxAcceleration) {
                this.mMaxAcceleration = a2.getFloat(index, this.mMaxAcceleration);
            } else if (index == C0179R.styleable.OnSwipe_moveWhenScrollAtTop) {
                this.mMoveWhenScrollAtTop = a2.getBoolean(index, this.mMoveWhenScrollAtTop);
            } else if (index == C0179R.styleable.OnSwipe_dragScale) {
                this.mDragScale = a2.getFloat(index, this.mDragScale);
            } else if (index == C0179R.styleable.OnSwipe_dragThreshold) {
                this.mDragThreshold = a2.getFloat(index, this.mDragThreshold);
            } else if (index == C0179R.styleable.OnSwipe_touchRegionId) {
                this.mTouchRegionId = a2.getResourceId(index, this.mTouchRegionId);
            } else if (index == C0179R.styleable.OnSwipe_onTouchUp) {
                this.mOnTouchUp = a2.getInt(index, this.mOnTouchUp);
            } else if (index == C0179R.styleable.OnSwipe_nestedScrollFlags) {
                this.mFlags = a2.getInteger(index, 0);
            } else if (index == C0179R.styleable.OnSwipe_limitBoundsTo) {
                this.mLimitBoundsTo = a2.getResourceId(index, 0);
            } else if (index == C0179R.styleable.OnSwipe_rotationCenterId) {
                this.mRotationCenterId = a2.getResourceId(index, this.mRotationCenterId);
            } else if (index == C0179R.styleable.OnSwipe_springDamping) {
                this.mSpringDamping = a2.getFloat(index, this.mSpringDamping);
            } else if (index == C0179R.styleable.OnSwipe_springMass) {
                this.mSpringMass = a2.getFloat(index, this.mSpringMass);
            } else if (index == C0179R.styleable.OnSwipe_springStiffness) {
                this.mSpringStiffness = a2.getFloat(index, this.mSpringStiffness);
            } else if (index == C0179R.styleable.OnSwipe_springStopThreshold) {
                this.mSpringStopThreshold = a2.getFloat(index, this.mSpringStopThreshold);
            } else if (index == C0179R.styleable.OnSwipe_springBoundary) {
                this.mSpringBoundary = a2.getInt(index, this.mSpringBoundary);
            } else if (index == C0179R.styleable.OnSwipe_autoCompleteMode) {
