...
class MotionConstrainedPoint implements Comparable<MotionConstrainedPoint> {
    ...
    void different(MotionConstrainedPoint points, boolean[] mask, String[] custom) {
        ...
        mask[1] = mask[1] | diff(this.f164x, points.f164x);
        mask[2] = mask[2] | diff(this.f165y, points.f165y);
    ...
    void fillStandard(double[] data, int[] toUse) {
        float[] fArr = {this.position, this.f164x, this.f165y, this.width, this.height, this.alpha, this.elevation, this.rotation, this.rotationX, this.rotationY, this.scaleX, this.scaleY, this.mPivotX, this.mPivotY, this.translationX, this.translationY, this.translationZ, this.mPathRotate};
        int i = 0;
        for (int i2 : toUse) {
            if (i2 < 18) {
                data[i] = fArr[r4];
