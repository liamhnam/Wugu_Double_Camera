...
public class ViewTransition {
    ...
    */
    ViewTransition(android.content.Context r10, org.xmlpull.p065v1.XmlPullParser r11) {
        /*
    ...
    private void parseViewTransitionTags(Context context, XmlPullParser parser) {
        TypedArray typedArrayObtainStyledAttributes = context.obtainStyledAttributes(Xml.asAttributeSet(parser), C0179R.styleable.ViewTransition);
        int indexCount = typedArrayObtainStyledAttributes.getIndexCount();
        for (int i = 0; i < indexCount; i++) {
            int index = typedArrayObtainStyledAttributes.getIndex(i);
            if (index == C0179R.styleable.ViewTransition_android_id) {
                this.mId = typedArrayObtainStyledAttributes.getResourceId(index, this.mId);
            } else if (index == C0179R.styleable.ViewTransition_motionTarget) {
    ...
    void applyTransition(ViewTransitionController controller, MotionLayout layout, int fromId, ConstraintSet current, final View... views) {
        ...
        layout.updateState(C0179R.id.view_transition, current);
        layout.setState(C0179R.id.view_transition, -1, -1);
        MotionScene.Transition transition = new MotionScene.Transition(-1, layout.mScene, C0179R.id.view_transition, fromId);
        ...
        layout.transitionToEnd(new Runnable() {
            ...
            public final void run() {
                this.f$0.m108x14d7500(views);
            }
        });
    }

    void m108x14d7500(View[] viewArr) {
        if (this.mSetsTag != -1) {
    ...
    private void updateTransition(MotionScene.Transition transition, View view) {
        ...
        if (keyFrames != null) {
            ...
            while (it.hasNext()) {
                keyFrames2.addKey(it.next().mo1499clone().setViewId(id));
            }
