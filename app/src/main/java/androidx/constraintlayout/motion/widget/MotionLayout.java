...
public class MotionLayout extends ConstraintLayout implements NestedScrollingParent3 {
    ...
    void setState(TransitionState newState) {
        ...
        int i = C01635.f166xabc7e4ac[transitionState.ordinal()];
        if (i == 1 || i == 2) {
    ...
    static class C01635 {
        ...
        static final int[] f166xabc7e4ac;
        ...
        static {
            ...
            try {
                f166xabc7e4ac[TransitionState.SETUP.ordinal()] = 2;
            } catch (NoSuchFieldError unused2) {
            }
            try {
                f166xabc7e4ac[TransitionState.MOVING.ordinal()] = 3;
            } catch (NoSuchFieldError unused3) {
            }
            try {
                f166xabc7e4ac[TransitionState.FINISHED.ordinal()] = 4;
    ...
    private static class MyTracker implements MotionTracker {
        ...
        private static MyTracker f167me = new MyTracker();
        ...
        public static MyTracker obtain() {
            f167me.tracker = VelocityTracker.obtain();
            return f167me;
        }
    ...
    */
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray typedArrayObtainStyledAttributes = getContext().obtainStyledAttributes(attrs, C0179R.styleable.MotionLayout);
            int indexCount = typedArrayObtainStyledAttributes.getIndexCount();
            boolean z = true;
            for (int i = 0; i < indexCount; i++) {
                int index = typedArrayObtainStyledAttributes.getIndex(i);
                if (index == C0179R.styleable.MotionLayout_layoutDescription) {
                    this.mScene = new MotionScene(getContext(), this, typedArrayObtainStyledAttributes.getResourceId(index, -1));
                } else if (index == C0179R.styleable.MotionLayout_currentState) {
                    this.mCurrentState = typedArrayObtainStyledAttributes.getResourceId(index, -1);
                } else if (index == C0179R.styleable.MotionLayout_motionProgress) {
                    this.mTransitionGoalPosition = typedArrayObtainStyledAttributes.getFloat(index, 0.0f);
                    this.mInTransition = true;
                } else if (index == C0179R.styleable.MotionLayout_applyMotionScene) {
                    z = typedArrayObtainStyledAttributes.getBoolean(index, z);
                } else if (index == C0179R.styleable.MotionLayout_showPaths) {
                    if (this.mDebugPath == 0) {
                        this.mDebugPath = typedArrayObtainStyledAttributes.getBoolean(index, false) ? 2 : 0;
                    }
                } else if (index == C0179R.styleable.MotionLayout_motionDebug) {
    ...
    public void updateStateAnimate(int stateId, ConstraintSet set, int duration) {
        if (this.mScene != null && this.mCurrentState == stateId) {
            updateState(C0179R.id.view_transition, getConstraintSet(stateId));
            setState(C0179R.id.view_transition, -1, -1);
            updateState(stateId, set);
            MotionScene.Transition transition = new MotionScene.Transition(-1, this.mScene, C0179R.id.view_transition, stateId);
