...
public abstract class ConstraintHelper extends View {
    ...
    protected void init(AttributeSet attrs) {
        if (attrs != null) {
            ...
            for (int i = 0; i < indexCount; i++) {
                ...
                if (index == C0179R.styleable.ConstraintLayout_Layout_constraint_referenced_ids) {
                    String string = typedArrayObtainStyledAttributes.getString(index);
                    this.mReferenceIds = string;
                    setIds(string);
                } else if (index == C0179R.styleable.ConstraintLayout_Layout_constraint_referenced_tags) {
    ...
    private int findId(String referenceId) {
        ...
        if (iFindId == 0) {
            try {
                iFindId = C0179R.id.class.getField(referenceId).getInt(null);
