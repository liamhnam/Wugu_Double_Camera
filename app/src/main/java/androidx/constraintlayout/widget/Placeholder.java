...
public class Placeholder extends View {
    ...
    private void init(AttributeSet attrs) {
        ...
        this.mContentId = -1;
        if (attrs != null) {
            TypedArray typedArrayObtainStyledAttributes = getContext().obtainStyledAttributes(attrs, C0179R.styleable.ConstraintLayout_placeholder);
            int indexCount = typedArrayObtainStyledAttributes.getIndexCount();
            for (int i = 0; i < indexCount; i++) {
                int index = typedArrayObtainStyledAttributes.getIndex(i);
                if (index == C0179R.styleable.ConstraintLayout_placeholder_content) {
                    this.mContentId = typedArrayObtainStyledAttributes.getResourceId(index, this.mContentId);
                } else if (index == C0179R.styleable.ConstraintLayout_placeholder_placeholder_emptyVisibility) {
    ...
    public void onDraw(Canvas canvas) {
        if (isInEditMode()) {
            ...
            paint.getTextBounds(LocationInfo.f2800NA, 0, 1, rect);
            canvas.drawText(LocationInfo.f2800NA, ((iWidth / 2.0f) - (rect.width() / 2.0f)) - rect.left, ((iHeight / 2.0f) + (rect.height() / 2.0f)) - rect.bottom, paint);
        }
    }
