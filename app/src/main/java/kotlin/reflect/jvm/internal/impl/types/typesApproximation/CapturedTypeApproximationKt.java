...
public final class CapturedTypeApproximationKt {
    ...
    public static final TypeProjection approximateCapturedTypesIfNecessary(TypeProjection typeProjection, boolean z) {
        ...
        if (!TypeUtils.contains(type, new Function1<UnwrappedType, Boolean>() {
            @Override
    ...
    private static final TypeProjection substituteCapturedTypesWithProjections(TypeProjection typeProjection) {
        TypeSubstitutor typeSubstitutorCreate = TypeSubstitutor.create(new TypeConstructorSubstitution() {
            @Override
