...
public class ZtlManagerA527 extends ZtlManager {
    ...
    public void setUSBtoPC(boolean z) {
        ...
        if (z) {
            systemProperty = getSystemProperty("ro.ztl.otgcmd", "");
            str = IcyHeaders.REQUEST_HEADER_ENABLE_METADATA_VALUE;
        } else {
            systemProperty = getSystemProperty("ro.ztl.hostcmd", "");
            str = ExifInterface.GPS_MEASUREMENT_2D;
        }
        if (systemProperty.length() > 0) {
            setSystemProperty("persist.usb.mode", str);
            System.out.println(systemProperty);
            execRootCmdSilent(systemProperty);
        }
    }

    @Override
    public boolean getUSBtoPC() {
        String systemProperty = getSystemProperty("persist.usb.mode", ExifInterface.GPS_MEASUREMENT_2D);
        System.out.println("A527 to PC:" + systemProperty);
        return systemProperty.equals(IcyHeaders.REQUEST_HEADER_ENABLE_METADATA_VALUE);
    }
