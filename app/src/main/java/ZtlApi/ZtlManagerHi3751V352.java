...
public class ZtlManagerHi3751V352 extends ZtlManager {
    ...
    public void setHDMIResolution(String str) {
        execRootCmdSilent("echo " + str + " > /sys/class/display/mode");
    }
    ...
    public int gpioStringToInt(String str) {
        if (!str.contains("GPIO")) {
            Log.e(this.TAG, "传入参数错误,请传入GPIO0_2之类的，实际以规格书为准");
            return -1;
        }
        return (511 - (Integer.parseInt(str.substring(4, str.indexOf("_"))) * 8)) - (8 - (8 - Integer.parseInt(str.substring(str.indexOf("_") + 1))));
    }
    ...
    public String[] getHDMIResolutions() {
        try {
            String strLoadFileAsString = loadFileAsString("/sys/class/amhdmitx/amhdmitx0/disp_cap");
            if (strLoadFileAsString.contains("*")) {
                strLoadFileAsString = strLoadFileAsString.replace("*", "");
            }
            String[] strArrSplit = strLoadFileAsString.split("\n");
            ArrayList arrayList = new ArrayList();
            for (String str : strArrSplit) {
                arrayList.add(new rotaionString(str));
            }
            Collections.sort(arrayList);
            int size = arrayList.size();
            String[] strArr = new String[size];
            for (int i = 0; i < size; i++) {
                strArr[i] = ((rotaionString) arrayList.get(i)).res;
            }
            return strArr;
        } catch (IOException e) {
            e.printStackTrace();
