...
public final class FlowableWindowBoundarySelector<T, B, V> extends AbstractFlowableWithUpstream<T, Flowable<T>> {
    ...
    static final class WindowBoundaryMainSubscriber<T, B, V> extends QueueDrainSubscriber<T, Object, Flowable<T>> implements Subscription {
        ...
        final List<UnicastProcessor<T>> f2617ws;
        ...
        public void onSubscribe(Subscription subscription) {
            if (SubscriptionHelper.validate(this.upstream, subscription)) {
                ...
                if (LifecycleKt$$ExternalSyntheticBackportWithForwarding0.m143m(this.boundary, null, operatorWindowBoundaryOpenSubscriber)) {
        ...
        public void onNext(T t) {
            ...
            if (fastEnter()) {
                Iterator<UnicastProcessor<T>> it = this.f2617ws.iterator();
                while (it.hasNext()) {
                    it.next().onNext(t);
                }
                if (leave(-1) == 0) {
        ...
        void drainLoop() {
            ...
            List<UnicastProcessor<T>> list = this.f2617ws;
            int iLeave = 1;
            while (true) {
                boolean z = this.done;
                ...
                if (!z2) {
                    if (objPoll instanceof WindowOperation) {
                        ...
                        if (windowOperation.f2618w != null) {
                            if (list.remove(windowOperation.f2618w)) {
                                windowOperation.f2618w.onComplete();
                                if (this.windows.decrementAndGet() == 0) {
                                    dispose();
                                    return;
                                }
        ...
        void close(OperatorWindowBoundaryCloseSubscriber<T, V> operatorWindowBoundaryCloseSubscriber) {
            ...
            this.queue.offer((U) new WindowOperation(operatorWindowBoundaryCloseSubscriber.f2616w, null));
            if (enter()) {
                drainLoop();
            }
        }
    ...
    static final class OperatorWindowBoundaryCloseSubscriber<T, V> extends DisposableSubscriber<V> {
        ...
        final UnicastProcessor<T> f2616w;
