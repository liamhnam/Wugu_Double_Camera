...
public final class FlowableRetryPredicate<T> extends AbstractFlowableWithUpstream<T, T> {
    ...
    static final class RetrySubscriber<T> extends AtomicInteger implements FlowableSubscriber<T> {
        ...
        final SubscriptionArbiter f2609sa;
        ...
        RetrySubscriber(Subscriber<? super T> subscriber, long j, Predicate<? super Throwable> predicate, SubscriptionArbiter subscriptionArbiter, Publisher<? extends T> publisher) {
            ...
            this.f2609sa = subscriptionArbiter;
        ...
        public void onSubscribe(Subscription subscription) {
            this.f2609sa.setSubscription(subscription);
        }
        ...
        void subscribeNext() {
            if (getAndIncrement() == 0) {
                int iAddAndGet = 1;
                while (!this.f2609sa.isCancelled()) {
                    long j = this.produced;
                    if (j != 0) {
                        this.produced = 0L;
                        this.f2609sa.produced(j);
                    }
