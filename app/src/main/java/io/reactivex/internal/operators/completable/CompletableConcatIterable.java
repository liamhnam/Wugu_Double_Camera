...
public final class CompletableConcatIterable extends Completable {
    ...
    public void subscribeActual(CompletableObserver completableObserver) {
        try {
            ...
            completableObserver.onSubscribe(concatInnerObserver.f2588sd);
    ...
    static final class ConcatInnerObserver extends AtomicInteger implements CompletableObserver {
        ...
        final SequentialDisposable f2588sd = new SequentialDisposable();
        ...
        public void onSubscribe(Disposable disposable) {
            this.f2588sd.replace(disposable);
        }
        ...
        void next() {
            if (!this.f2588sd.isDisposed() && getAndIncrement() == 0) {
                Iterator<? extends CompletableSource> it = this.sources;
                while (!this.f2588sd.isDisposed()) {
                    try {
                        if (!it.hasNext()) {
