...
public class PDLayoutAttributeObject extends PDStandardAttributeObject {
    ...
    public String toString() {
        ...
        if (isSpecified(PLACEMENT)) {
            sbAppend.append(", Placement=").append(getPlacement());
        }
        if (isSpecified(WRITING_MODE)) {
            sbAppend.append(", WritingMode=").append(getWritingMode());
        }
        if (isSpecified(BACKGROUND_COLOR)) {
            sbAppend.append(", BackgroundColor=").append(getBackgroundColor());
        }
        if (isSpecified(BORDER_COLOR)) {
            sbAppend.append(", BorderColor=").append(getBorderColors());
        }
        if (isSpecified(BORDER_STYLE)) {
            Object borderStyle = getBorderStyle();
            sbAppend.append(", BorderStyle=");
            if (borderStyle instanceof String[]) {
                sbAppend.append(arrayToString((String[]) borderStyle));
            } else {
                sbAppend.append(borderStyle);
            }
        }
        if (isSpecified(BORDER_THICKNESS)) {
            ...
            sbAppend.append(", BorderThickness=");
            if (borderThickness instanceof float[]) {
                sbAppend.append(arrayToString((float[]) borderThickness));
            } else {
                sbAppend.append(String.valueOf(borderThickness));
            }
        }
        if (isSpecified(PADDING)) {
            Object padding = getPadding();
            sbAppend.append(", Padding=");
            if (padding instanceof float[]) {
                sbAppend.append(arrayToString((float[]) padding));
            } else {
                sbAppend.append(String.valueOf(padding));
            }
        }
        if (isSpecified(COLOR)) {
            sbAppend.append(", Color=").append(getColor());
        }
        if (isSpecified(SPACE_BEFORE)) {
            sbAppend.append(", SpaceBefore=").append(String.valueOf(getSpaceBefore()));
        }
        if (isSpecified(SPACE_AFTER)) {
            sbAppend.append(", SpaceAfter=").append(String.valueOf(getSpaceAfter()));
        }
        if (isSpecified(START_INDENT)) {
            sbAppend.append(", StartIndent=").append(String.valueOf(getStartIndent()));
        }
        if (isSpecified(END_INDENT)) {
            sbAppend.append(", EndIndent=").append(String.valueOf(getEndIndent()));
        }
        if (isSpecified(TEXT_INDENT)) {
            sbAppend.append(", TextIndent=").append(String.valueOf(getTextIndent()));
        }
        if (isSpecified(TEXT_ALIGN)) {
            sbAppend.append(", TextAlign=").append(getTextAlign());
        }
        if (isSpecified(BBOX)) {
            sbAppend.append(", BBox=").append(getBBox());
        }
        if (isSpecified(WIDTH)) {
            Object width = getWidth();
            sbAppend.append(", Width=");
            if (width instanceof Float) {
                sbAppend.append(String.valueOf(width));
            } else {
                sbAppend.append(width);
            }
        }
        if (isSpecified(HEIGHT)) {
            ...
            sbAppend.append(", Height=");
            if (height instanceof Float) {
                sbAppend.append(String.valueOf(height));
            } else {
                sbAppend.append(height);
            }
        }
        if (isSpecified(BLOCK_ALIGN)) {
            sbAppend.append(", BlockAlign=").append(getBlockAlign());
        }
        if (isSpecified(INLINE_ALIGN)) {
            sbAppend.append(", InlineAlign=").append(getInlineAlign());
        }
        if (isSpecified(T_BORDER_STYLE)) {
            Object tBorderStyle = getTBorderStyle();
            sbAppend.append(", TBorderStyle=");
            if (tBorderStyle instanceof String[]) {
                sbAppend.append(arrayToString((String[]) tBorderStyle));
            } else {
                sbAppend.append(tBorderStyle);
            }
        }
        if (isSpecified(T_PADDING)) {
            ...
            sbAppend.append(", TPadding=");
            if (tPadding instanceof float[]) {
                sbAppend.append(arrayToString((float[]) tPadding));
            } else {
                sbAppend.append(String.valueOf(tPadding));
            }
        }
        if (isSpecified(BASELINE_SHIFT)) {
            sbAppend.append(", BaselineShift=").append(String.valueOf(getBaselineShift()));
        }
        if (isSpecified(LINE_HEIGHT)) {
            Object lineHeight = getLineHeight();
            sbAppend.append(", LineHeight=");
            if (lineHeight instanceof Float) {
                sbAppend.append(String.valueOf(lineHeight));
            } else {
                sbAppend.append(lineHeight);
            }
        }
        if (isSpecified(TEXT_DECORATION_COLOR)) {
            sbAppend.append(", TextDecorationColor=").append(getTextDecorationColor());
        }
        if (isSpecified(TEXT_DECORATION_THICKNESS)) {
            sbAppend.append(", TextDecorationThickness=").append(String.valueOf(getTextDecorationThickness()));
        }
        if (isSpecified(TEXT_DECORATION_TYPE)) {
            sbAppend.append(", TextDecorationType=").append(getTextDecorationType());
        }
        if (isSpecified(RUBY_ALIGN)) {
            sbAppend.append(", RubyAlign=").append(getRubyAlign());
        }
        if (isSpecified(RUBY_POSITION)) {
            sbAppend.append(", RubyPosition=").append(getRubyPosition());
        }
        if (isSpecified(GLYPH_ORIENTATION_VERTICAL)) {
            sbAppend.append(", GlyphOrientationVertical=").append(getGlyphOrientationVertical());
        }
        if (isSpecified(COLUMN_COUNT)) {
            sbAppend.append(", ColumnCount=").append(String.valueOf(getColumnCount()));
        }
        if (isSpecified(COLUMN_GAP)) {
            Object columnGap = getColumnGap();
            sbAppend.append(", ColumnGap=");
            if (columnGap instanceof float[]) {
                sbAppend.append(arrayToString((float[]) columnGap));
            } else {
                sbAppend.append(String.valueOf(columnGap));
            }
        }
        if (isSpecified(COLUMN_WIDTHS)) {
            Object columnWidths = getColumnWidths();
            sbAppend.append(", ColumnWidths=");
            if (columnWidths instanceof float[]) {
                sbAppend.append(arrayToString((float[]) columnWidths));
            } else {
                sbAppend.append(String.valueOf(columnWidths));
            }
        }
