...
class EngineJob<R> implements DecodeJob.Callback<R>, FactoryPools.Poolable {
    ...
    void notifyCallbacksOfResult() {
        synchronized (this) {
            ...
            for (ResourceCallbackAndExecutor resourceCallbackAndExecutor : resourceCallbacksAndExecutorsCopy) {
                resourceCallbackAndExecutor.executor.execute(new CallResourceReady(resourceCallbackAndExecutor.f543cb));
            }
    ...
    void notifyCallbacksOfException() {
        synchronized (this) {
            ...
            for (ResourceCallbackAndExecutor resourceCallbackAndExecutor : resourceCallbacksAndExecutorsCopy) {
                resourceCallbackAndExecutor.executor.execute(new CallLoadFailed(resourceCallbackAndExecutor.f543cb));
            }
            decrementPendingCallbacks();
        }
    }
    ...
    private class CallLoadFailed implements Runnable {
        ...
        private final ResourceCallback f541cb;

        CallLoadFailed(ResourceCallback resourceCallback) {
            this.f541cb = resourceCallback;
        }

        @Override
        public void run() {
            synchronized (this.f541cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f541cb)) {
                        EngineJob.this.callCallbackOnLoadFailed(this.f541cb);
                    }
    ...
    private class CallResourceReady implements Runnable {
        ...
        private final ResourceCallback f542cb;

        CallResourceReady(ResourceCallback resourceCallback) {
            this.f542cb = resourceCallback;
        }

        @Override
        public void run() {
            synchronized (this.f542cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f542cb)) {
                        EngineJob.this.engineResource.acquire();
                        EngineJob.this.callCallbackOnResourceReady(this.f542cb);
                        EngineJob.this.removeCallback(this.f542cb);
                    }
                    EngineJob.this.decrementPendingCallbacks();
                }
            }
    ...
    static final class ResourceCallbackAndExecutor {
        ...
        final ResourceCallback f543cb;
        final Executor executor;

        ResourceCallbackAndExecutor(ResourceCallback resourceCallback, Executor executor) {
            this.f543cb = resourceCallback;
            this.executor = executor;
        }

        public boolean equals(Object obj) {
            if (obj instanceof ResourceCallbackAndExecutor) {
                return this.f543cb.equals(((ResourceCallbackAndExecutor) obj).f543cb);
            }
            return false;
        }

        public int hashCode() {
            return this.f543cb.hashCode();
        }
    }
