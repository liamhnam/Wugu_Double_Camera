...
public final class SimpleCache implements Cache {
            ...
            if (databaseProvider != null) {
                ...
                if (jLoadUid != -1) {
                    try {
                        ...
                        Log.m346w(TAG, "Failed to delete file metadata: " + jLoadUid);
                    }
                    try {
    ...
    public synchronized void release() {
        ...
        try {
            try {
                ...
                Log.m343e(TAG, "Storing index file failed", e);
                unlockFolder(this.cacheDir);
            }
            this.released = true;
        } catch (Throwable th) {
    ...
    public synchronized CacheSpan startReadWrite(String str, long j) throws InterruptedException, Cache.CacheException {
        ...
        checkInitialization();
        while (true) {
            cacheSpanStartReadWriteNonBlocking = startReadWriteNonBlocking(str, j);
    ...
    public synchronized File startFile(String str, long j, long j2) throws Cache.CacheException {
        ...
        return SimpleCacheSpan.getCacheFile(file, cachedContent.f594id, j, System.currentTimeMillis());
    }
    ...
    private SimpleCacheSpan touchSpan(String str, SimpleCacheSpan simpleCacheSpan) {
        ...
        if (cacheFileMetadataIndex != null) {
            try {
                ...
                Log.m346w(TAG, "Failed to update index with new touch timestamp.");
            }
            z = false;
        } else {
            z = true;
    ...
    private void removeSpanInternal(CacheSpan cacheSpan) {
        ...
        if (this.fileIndex != null) {
            ...
            try {
                ...
                Log.m346w(TAG, "Failed to remove file index entry for: " + name);
            }
        }
        this.contentIndex.maybeRemove(cachedContent.key);
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (name.endsWith(UID_FILE_SUFFIX)) {
                try {
                    ...
                    Log.m342e(TAG, "Malformed UID file: " + file);
                    file.delete();
                }
            }
        }
