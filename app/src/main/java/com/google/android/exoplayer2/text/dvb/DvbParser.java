...
final class DvbParser {
    ...
    private static final byte[] defaultMap2To4 = {0, 7, 8, SnmpConstants.SNMP_ERR_UNDOFAILED};
    private static final byte[] defaultMap2To8 = {0, 119, -120, -1};
    private static final byte[] defaultMap4To8 = {0, 17, 34, 51, SnmpConstants.OPAQUE, 85, 102, 119, -120, -103, -86, -69, -52, -35, -18, -1};
    ...
    public List<Cue> decode(byte[] bArr, int i) {
        ...
        for (int i4 = 0; i4 < sparseArray2.size(); i4++) {
            ...
            arrayList.add(new Cue(Bitmap.createBitmap(this.bitmap, i5, i6, regionComposition.width, regionComposition.height), i5 / displayDefinition.width, 0, i6 / displayDefinition.height, 0, regionComposition.width / displayDefinition.width, regionComposition.height / displayDefinition.height));
    ...
    private static void parseSubtitlingSegment(ParsableBitArray parsableBitArray, SubtitleService subtitleService) {
        ...
        if (bits3 * 8 > parsableBitArray.bitsLeft()) {
            Log.m346w(TAG, "Data field length exceeds limit");
        ...
        switch (bits) {
            ...
            case 17:
                ...
                if (bits2 == subtitleService.subtitlePageId && pageComposition3 != null) {
                    ...
                    if (pageComposition3.state == 0 && (regionComposition = subtitleService.regions.get(regionComposition2.f589id)) != null) {
                        regionComposition2.mergeFrom(regionComposition);
                    }
                    subtitleService.regions.put(regionComposition2.f589id, regionComposition2);
                }
                break;
            case 18:
                if (bits2 == subtitleService.subtitlePageId) {
                    ClutDefinition clutDefinition = parseClutDefinition(parsableBitArray, bits3);
                    subtitleService.cluts.put(clutDefinition.f587id, clutDefinition);
                } else if (bits2 == subtitleService.ancillaryPageId) {
                    ClutDefinition clutDefinition2 = parseClutDefinition(parsableBitArray, bits3);
                    subtitleService.ancillaryCluts.put(clutDefinition2.f587id, clutDefinition2);
                }
                break;
            case 19:
                if (bits2 == subtitleService.subtitlePageId) {
                    ObjectData objectData = parseObjectData(parsableBitArray);
                    subtitleService.objects.put(objectData.f588id, objectData);
                } else if (bits2 == subtitleService.ancillaryPageId) {
                    ObjectData objectData2 = parseObjectData(parsableBitArray);
                    subtitleService.ancillaryObjects.put(objectData2.f588id, objectData2);
                }
                break;
            case 20:
                if (bits2 == subtitleService.subtitlePageId) {
    ...
    private static int[] generateDefault8BitClutEntries() {
        ...
        for (int i2 = 0; i2 < 256; i2++) {
            if (i2 < 8) {
                ...
                int i3 = i2 & SyslogAppender.LOG_LOCAL1;
                int i4 = PowerState.Code.resetInit;
                if (i3 == 0) {
