...
final class MediaPeriodQueue {
    ...
    private boolean areDurationsCompatible(long j, long j2) {
        return j == C1041C.TIME_UNSET || j == j2;
    }
    ...
    public boolean shouldLoadNextMediaPeriod() {
        MediaPeriodHolder mediaPeriodHolder = this.loading;
        return mediaPeriodHolder == null || (!mediaPeriodHolder.info.isFinal && this.loading.isFullyBuffered() && this.loading.info.durationUs != C1041C.TIME_UNSET && this.length < 100);
    }
    ...
    public MediaPeriodHolder enqueueNextMediaPeriodHolder(RendererCapabilities[] rendererCapabilitiesArr, TrackSelector trackSelector, Allocator allocator, MediaSource mediaSource, MediaPeriodInfo mediaPeriodInfo, TrackSelectorResult trackSelectorResult) {
        ...
        MediaPeriodHolder mediaPeriodHolder = this.loading;
        if (mediaPeriodHolder == null) {
            rendererOffset = (!mediaPeriodInfo.f566id.isAd() || mediaPeriodInfo.contentPositionUs == C1041C.TIME_UNSET) ? 0L : mediaPeriodInfo.contentPositionUs;
        } else {
    ...
    public MediaPeriodHolder advancePlayingPeriod() {
        ...
        if (i == 0) {
            ...
            this.oldFrontPeriodWindowSequenceNumber = this.playing.info.f566id.windowSequenceNumber;
        }
    ...
    public void clear(boolean z) {
        ...
        if (mediaPeriodHolder != null) {
            ...
            this.oldFrontPeriodWindowSequenceNumber = mediaPeriodHolder.info.f566id.windowSequenceNumber;
    ...
    public boolean updateQueuedPeriods(long j, long j2) {
        ...
        while (next != null) {
            ...
            if (!areDurationsCompatible(mediaPeriodInfo.durationUs, updatedMediaPeriodInfo.durationUs)) {
                return (removeAfter(next) || (next == this.reading && ((j2 > Long.MIN_VALUE ? 1 : (j2 == Long.MIN_VALUE ? 0 : -1)) == 0 || (j2 > ((updatedMediaPeriodInfo.durationUs > C1041C.TIME_UNSET ? 1 : (updatedMediaPeriodInfo.durationUs == C1041C.TIME_UNSET ? 0 : -1)) == 0 ? Long.MAX_VALUE : next.toRendererTime(updatedMediaPeriodInfo.durationUs)) ? 1 : (j2 == ((updatedMediaPeriodInfo.durationUs > C1041C.TIME_UNSET ? 1 : (updatedMediaPeriodInfo.durationUs == C1041C.TIME_UNSET ? 0 : -1)) == 0 ? Long.MAX_VALUE : next.toRendererTime(updatedMediaPeriodInfo.durationUs)) ? 0 : -1)) >= 0))) ? false : true;
            }
    ...
    public MediaPeriodInfo getUpdatedMediaPeriodInfo(MediaPeriodInfo mediaPeriodInfo) {
        long durationUs;
        MediaSource.MediaPeriodId mediaPeriodId = mediaPeriodInfo.f566id;
        ...
        this.timeline.getPeriodByUid(mediaPeriodInfo.f566id.periodUid, this.period);
        if (mediaPeriodId.isAd()) {
            durationUs = this.period.getAdDurationUs(mediaPeriodId.adGroupIndex, mediaPeriodId.adIndexInAdGroup);
        } else {
            durationUs = (mediaPeriodInfo.endPositionUs == C1041C.TIME_UNSET || mediaPeriodInfo.endPositionUs == Long.MIN_VALUE) ? this.period.getDurationUs() : mediaPeriodInfo.endPositionUs;
        }
    ...
    private long resolvePeriodIndexToWindowSequenceNumber(Object obj) {
        ...
        for (MediaPeriodHolder next = this.playing; next != null; next = next.getNext()) {
            if (next.uid.equals(obj)) {
                return next.info.f566id.windowSequenceNumber;
            }
        }
        for (MediaPeriodHolder next2 = this.playing; next2 != null; next2 = next2.getNext()) {
            ...
            if (indexOfPeriod2 != -1 && this.timeline.getPeriod(indexOfPeriod2, this.period).windowIndex == i) {
                return next2.info.f566id.windowSequenceNumber;
            }
        }
    ...
    private boolean canKeepMediaPeriodHolder(MediaPeriodInfo mediaPeriodInfo, MediaPeriodInfo mediaPeriodInfo2) {
        return mediaPeriodInfo.startPositionUs == mediaPeriodInfo2.startPositionUs && mediaPeriodInfo.f566id.equals(mediaPeriodInfo2.f566id);
    }
    ...
    private MediaPeriodInfo getFollowingMediaPeriodInfo(MediaPeriodHolder mediaPeriodHolder, long j) {
        ...
        if (mediaPeriodInfo.isLastInTimelinePeriod) {
            int nextPeriodIndex = this.timeline.getNextPeriodIndex(this.timeline.getIndexOfPeriod(mediaPeriodInfo.f566id.periodUid), this.period, this.window, this.repeatMode, this.shuffleModeEnabled);
            if (nextPeriodIndex == -1) {
                return null;
            }
            int i = this.timeline.getPeriod(nextPeriodIndex, this.period, true).windowIndex;
            Object obj2 = this.period.uid;
            long j6 = mediaPeriodInfo.f566id.windowSequenceNumber;
            if (this.timeline.getWindow(i, this.window).firstPeriodIndex == nextPeriodIndex) {
                Pair<Object, Long> periodPosition = this.timeline.getPeriodPosition(this.window, this.period, i, C1041C.TIME_UNSET, Math.max(0L, rendererOffset));
        ...
        if (mediaPeriodId.isAd()) {
            ...
            if (j7 == C1041C.TIME_UNSET) {
                Timeline timeline = this.timeline;
                Timeline.Window window = this.window;
                Timeline.Period period = this.period;
                Pair<Object, Long> periodPosition2 = timeline.getPeriodPosition(window, period, period.windowIndex, C1041C.TIME_UNSET, Math.max(0L, rendererOffset));
    ...
    private MediaPeriodInfo getMediaPeriodInfoForAd(Object obj, int i, int i2, long j, long j2) {
        ...
        return new MediaPeriodInfo(mediaPeriodId, i2 == this.period.getFirstAdIndexToPlay(i) ? this.period.getAdResumePositionUs() : 0L, j, C1041C.TIME_UNSET, this.timeline.getPeriodByUid(mediaPeriodId.periodUid, this.period).getAdDurationUs(mediaPeriodId.adGroupIndex, mediaPeriodId.adIndexInAdGroup), false, false);
    }
    ...
    private MediaPeriodInfo getMediaPeriodInfoForContent(Object obj, long j, long j2) {
        ...
        return new MediaPeriodInfo(mediaPeriodId, j, C1041C.TIME_UNSET, adGroupTimeUs, (adGroupTimeUs == C1041C.TIME_UNSET || adGroupTimeUs == Long.MIN_VALUE) ? this.period.durationUs : adGroupTimeUs, zIsLastInPeriod, zIsLastInTimeline);
    }
