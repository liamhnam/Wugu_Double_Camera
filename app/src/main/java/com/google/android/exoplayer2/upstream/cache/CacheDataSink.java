...
public final class CacheDataSink implements DataSink {
    ...
    public CacheDataSink(Cache cache, long j, int i) {
        ...
        if (j != -1 && j < MIN_RECOMMENDED_FRAGMENT_SIZE) {
            Log.m346w(TAG, "fragmentSize is below the minimum recommended value of 2097152. This may cause poor cache performance.");
        }
    ...
    private void openNextOutputStream() throws IOException {
        this.file = this.cache.startFile(this.dataSpec.key, this.dataSpec.absoluteStreamPosition + this.dataSpecBytesWritten, this.dataSpec.length != -1 ? Math.min(this.dataSpec.length - this.dataSpecBytesWritten, this.dataSpecFragmentSize) : -1L);
