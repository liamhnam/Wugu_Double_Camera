...
final class AudioTrackPositionTracker {
    ...
    public void setAudioTrack(AudioTrack audioTrack, int i, int i2, int i3) {
        ...
        this.hasData = false;
        this.stopTimestampUs = C1041C.TIME_UNSET;
        this.forceResetWorkaroundTimeMs = C1041C.TIME_UNSET;
        this.latencyUs = 0L;
    }
    ...
    public boolean mayHandleBuffer(long j) {
        ...
        if (z && !zHasPendingData && playState != 1 && (listener = this.listener) != null) {
            listener.onUnderrun(this.bufferSize, C1041C.usToMs(this.bufferSizeUs));
        }
        return true;
    }
    ...
    public boolean isStalled(long j) {
        return this.forceResetWorkaroundTimeMs != C1041C.TIME_UNSET && j > 0 && SystemClock.elapsedRealtime() - this.forceResetWorkaroundTimeMs >= FORCE_RESET_WORKAROUND_TIMEOUT_MS;
    }
    ...
    public boolean pause() {
        resetSyncParams();
        if (this.stopTimestampUs != C1041C.TIME_UNSET) {
            return false;
        }
        ((AudioTimestampPoller) Assertions.checkNotNull(this.audioTimestampPoller)).reset();
        return true;
    ...
    private long getPlaybackHeadPosition() {
        ...
        if (Util.SDK_INT <= 29) {
            if (playbackHeadPosition == 0 && this.lastRawPlaybackHeadPosition > 0 && playState == 3) {
                if (this.forceResetWorkaroundTimeMs == C1041C.TIME_UNSET) {
