...
public final class FlacStreamMetadata {
    ...
    public long getDurationUs() {
        ...
        return j == 0 ? C1041C.TIME_UNSET : (j * 1000000) / ((long) this.sampleRate);
    }
    ...
    public Format getFormat(byte[] bArr, Metadata metadata) {
        ...
        return Format.createAudioSampleFormat(null, MimeTypes.AUDIO_FLAC, null, getBitRate(), i, this.channels, this.sampleRate, -1, 0, 0, Collections.singletonList(bArr), null, 0, null, metadataCopyWithAppendedEntriesFrom);
    }
    ...
    private static Metadata buildMetadata(List<String> list, List<PictureFrame> list2) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (strArrSplitAtFirst.length != 2) {
                Log.m346w(TAG, "Failed to parse Vorbis comment: " + str);
            } else {
