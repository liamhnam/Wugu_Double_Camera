...
public final class Cea708Decoder extends CeaDecoder {
            ...
            if (i == 2 || i == 3) {
                if (z) {
                    if (i == 3) {
                        ...
                        int i2 = (unsignedByte2 & SnmpConstants.ASN_PRIVATE) >> 6;
                        ...
                        if (dtvCcPacket3 == null) {
                            Log.m342e(TAG, "Encountered DTVCC_PACKET_DATA before DTVCC_PACKET_START");
                        } else {
    ...
    private void processCurrentPacket() {
        if (this.currentDtvCcPacket.currentIndex != (this.currentDtvCcPacket.packetSize * 2) - 1) {
            Log.m346w(TAG, "DtvCcPacket ended prematurely; size is " + ((this.currentDtvCcPacket.packetSize * 2) - 1) + ", but current index is " + this.currentDtvCcPacket.currentIndex + " (sequence number " + this.currentDtvCcPacket.sequenceNumber + "); ignoring packet");
            return;
        }
        ...
        if (bits == 7) {
            ...
            if (bits < 7) {
                Log.m346w(TAG, "Invalid extended service number: " + bits);
            }
        }
        if (bits2 == 0) {
            if (bits != 0) {
                Log.m346w(TAG, "serviceNumber is non-zero (" + bits + ") when blockSize is 0");
                return;
            }
            return;
        }
            ...
            if (bits3 == 16) {
                ...
                if (bits4 <= 31) {
                    ...
                    if (bits4 <= 127) {
                        ...
                        Log.m346w(TAG, "Invalid extended command: " + bits4);
                    }
                    z = true;
                }
            } else if (bits3 <= 31) {
                ...
                if (bits3 <= 127) {
                    ...
                    Log.m346w(TAG, "Invalid base command: " + bits3);
                }
                z = true;
            }
        }
    ...
    private void handleC0Command(int i) {
        if (i != 0) {
            ...
            switch (i) {
                ...
                default:
                    if (i >= 17 && i <= 23) {
                        Log.m346w(TAG, "Currently unsupported COMMAND_EXT1 Command: " + i);
                        this.serviceBlockPacket.skipBits(8);
                    } else if (i >= 24 && i <= 31) {
                        Log.m346w(TAG, "Currently unsupported COMMAND_P16 Command: " + i);
                        this.serviceBlockPacket.skipBits(16);
                    } else {
                        Log.m346w(TAG, "Invalid C0 command: " + i);
                    }
                    break;
            }
        }
    ...
    private void handleC1Command(int i) {
        ...
        switch (i) {
            ...
            default:
                Log.m346w(TAG, "Invalid C1 command: " + i);
                break;
            case COMMAND_SWA:
                if (!this.currentCueBuilder.isDefined()) {
                ...
                default:
                    switch (i) {
                        ...
                        case 127:
                            ...
                            break;
                        default:
                            Log.m346w(TAG, "Invalid G2 character: " + i);
                            break;
                    }
                    break;
            }
    ...
    private void handleG3Character(int i) {
        if (i == 160) {
            ...
            Log.m346w(TAG, "Invalid G3 character: " + i);
