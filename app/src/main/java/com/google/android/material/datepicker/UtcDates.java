...
class UtcDates {
    ...
    static String getDefaultTextInputHint(Resources resources, SimpleDateFormat simpleDateFormat) {
        ...
        String string = resources.getString(C1156R.string.mtrl_picker_text_input_year_abbr);
        String string2 = resources.getString(C1156R.string.mtrl_picker_text_input_month_abbr);
        String string3 = resources.getString(C1156R.string.mtrl_picker_text_input_day_abbr);
        if (pattern.replaceAll("[^y]", "").length() == 1) {
            pattern = pattern.replace("y", "yyyy");
        }
        return pattern.replace(Media.f728d, string3).replace("M", string2).replace("y", string);
    }
