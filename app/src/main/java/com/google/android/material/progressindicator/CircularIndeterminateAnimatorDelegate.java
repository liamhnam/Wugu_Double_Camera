...
final class CircularIndeterminateAnimatorDelegate extends IndeterminateAnimatorDelegate<ObjectAnimator> {
    ...
    private static final int[] DELAY_TO_COLLAPSE_IN_MS = {UiPosIndexEnum.PRINT_SIGN_BG, 2017, 3367, 4717};
    ...
    private void updateSegmentPositions(int i) {
        ...
        for (int i2 = 0; i2 < 4; i2++) {
            float fractionInRange = getFractionInRange(i, DELAY_TO_EXPAND_IN_MS[i2], UiPosIndexEnum.PRINT_SIGN_BG);
            float[] fArr = this.segmentPositions;
            fArr[1] = fArr[1] + (this.interpolator.getInterpolation(fractionInRange) * 250.0f);
            float fractionInRange2 = getFractionInRange(i, DELAY_TO_COLLAPSE_IN_MS[i2], UiPosIndexEnum.PRINT_SIGN_BG);
    ...
    private void maybeUpdateSegmentColors(int i) {
        for (int i2 = 0; i2 < 4; i2++) {
            float fractionInRange = getFractionInRange(i, DELAY_TO_FADE_IN_MS[i2], UiPosIndexEnum.PAYMENT_DOU_YIN_IC);
