...
public final class BadgeState {
    ...
    BadgeState(Context context, int i, int i2, int i3, State state) {
        ...
        this.badgeRadius = typedArrayGenerateTypedArray.getDimensionPixelSize(C1156R.styleable.Badge_badgeRadius, resources.getDimensionPixelSize(C1156R.dimen.mtrl_badge_radius));
        this.badgeWidePadding = typedArrayGenerateTypedArray.getDimensionPixelSize(C1156R.styleable.Badge_badgeWidePadding, resources.getDimensionPixelSize(C1156R.dimen.mtrl_badge_long_text_horizontal_padding));
        this.badgeWithTextRadius = typedArrayGenerateTypedArray.getDimensionPixelSize(C1156R.styleable.Badge_badgeWithTextRadius, resources.getDimensionPixelSize(C1156R.dimen.mtrl_badge_with_text_radius));
        state2.alpha = state.alpha == -2 ? 255 : state.alpha;
        if (state.contentDescriptionNumberless == null) {
            string = context.getString(C1156R.string.mtrl_badge_numberless_content_description);
        } else {
        ...
        if (state.contentDescriptionQuantityStrings == 0) {
            i4 = C1156R.plurals.mtrl_badge_content_description;
        } else {
        ...
        if (state.contentDescriptionExceedsMaxBadgeNumberRes == 0) {
            i5 = C1156R.string.mtrl_exceed_max_badge_number_content_description;
        } else {
        ...
        if (state.maxCharacterCount == -2) {
            i6 = typedArrayGenerateTypedArray.getInt(C1156R.styleable.Badge_maxCharacterCount, 4);
        } else {
            i6 = state.maxCharacterCount;
        }
        state2.maxCharacterCount = i6;
        if (state.number == -2) {
            if (typedArrayGenerateTypedArray.hasValue(C1156R.styleable.Badge_number)) {
                state2.number = typedArrayGenerateTypedArray.getInt(C1156R.styleable.Badge_number, 0);
            } else {
                state2.number = -1;
            }
        } else {
        ...
        if (state.backgroundColor == null) {
            iIntValue = readColorFromAttributes(context, typedArrayGenerateTypedArray, C1156R.styleable.Badge_backgroundColor);
        } else {
            iIntValue = state.backgroundColor.intValue();
        }
        state2.backgroundColor = Integer.valueOf(iIntValue);
        if (state.badgeTextColor == null) {
            if (typedArrayGenerateTypedArray.hasValue(C1156R.styleable.Badge_badgeTextColor)) {
                state2.badgeTextColor = Integer.valueOf(readColorFromAttributes(context, typedArrayGenerateTypedArray, C1156R.styleable.Badge_badgeTextColor));
            } else {
                state2.badgeTextColor = Integer.valueOf(new TextAppearance(context, C1156R.style.TextAppearance_MaterialComponents_Badge).getTextColor().getDefaultColor());
            }
        } else {
            state2.badgeTextColor = state.badgeTextColor;
        }
        if (state.badgeGravity == null) {
            iIntValue2 = typedArrayGenerateTypedArray.getInt(C1156R.styleable.Badge_badgeGravity, 8388661);
        } else {
        ...
        if (state.horizontalOffsetWithoutText == null) {
            iIntValue3 = typedArrayGenerateTypedArray.getDimensionPixelOffset(C1156R.styleable.Badge_horizontalOffset, 0);
        } else {
        ...
        if (state.verticalOffsetWithoutText == null) {
            iIntValue4 = typedArrayGenerateTypedArray.getDimensionPixelOffset(C1156R.styleable.Badge_verticalOffset, 0);
        } else {
        ...
        if (state.horizontalOffsetWithText == null) {
            iIntValue5 = typedArrayGenerateTypedArray.getDimensionPixelOffset(C1156R.styleable.Badge_horizontalOffsetWithText, state2.horizontalOffsetWithoutText.intValue());
        } else {
        ...
        if (state.verticalOffsetWithText == null) {
            iIntValue6 = typedArrayGenerateTypedArray.getDimensionPixelOffset(C1156R.styleable.Badge_verticalOffsetWithText, state2.verticalOffsetWithoutText.intValue());
        } else {
    ...
    private TypedArray generateTypedArray(Context context, int i, int i2, int i3) {
        ...
        return ThemeEnforcement.obtainStyledAttributes(context, attributeSet, C1156R.styleable.Badge, i2, styleAttribute == 0 ? i3 : styleAttribute, new int[0]);
    }
