...
abstract class DateFormatTextWatcher extends TextWatcherAdapter {
    ...
    DateFormatTextWatcher(final String str, DateFormat dateFormat, TextInputLayout textInputLayout, CalendarConstraints calendarConstraints) {
        ...
        this.outOfRange = textInputLayout.getContext().getString(C1156R.string.mtrl_picker_out_of_range);
        this.setErrorCallback = new Runnable() {
            @Override
            public final void run() {
                this.f$0.m364x5657fb8e(str);
            }
        };
    }

    void m364x5657fb8e(String str) {
        TextInputLayout textInputLayout = this.textInputLayout;
        DateFormat dateFormat = this.dateFormat;
        Context context = textInputLayout.getContext();
        textInputLayout.setError(context.getString(C1156R.string.mtrl_picker_invalid_format) + "\n" + String.format(context.getString(C1156R.string.mtrl_picker_invalid_format_use), sanitizeDateString(str)) + "\n" + String.format(context.getString(C1156R.string.mtrl_picker_invalid_format_example), sanitizeDateString(dateFormat.format(new Date(UtcDates.getTodayCalendar().getTimeInMillis())))));
        onInvalidDate();
    }
    ...
    private Runnable createRangeErrorCallback(final long j) {
        return new Runnable() {
            @Override
            public final void run() {
                this.f$0.m363x14d77527(j);
            }
        };
    }

    void m363x14d77527(long j) {
